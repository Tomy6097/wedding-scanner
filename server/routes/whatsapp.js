        ...
        res.on('end', async () => {
          ...
          try {
            ...
            if (res.statusCode === 429 && retriesLeft > 0) {
              const wait = 60000;
              console.log('[EF] 429 rate limited - waiting 60s then retrying (' + retriesLeft + ' left)');
...
// In-flight lock to prevent double-sends
      ...
      try {
        ...
        if (type === 'qr' || type === 'invite') {
          ...
          if (isFailed) {
            ...
            if (type === 'qr') { g.sms_sent = true; g.sms_sent_at = new Date(); }
            await g.save();
            sent++;
          }
...
router.get('/unsent/:event_id', requireAdmin, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { total_with_phone: total, wa_sent: sent, wa_unsent: guests.length, wa_failed: failed, guests }
    });
  } catch (err) {
    res.status(500).json({ error: String(err.message || err) });
  }
